...
func (r *TeamCompositionRepository) All() (tcs []model.TeamComposition, err error) {
	return tcs, r.store.db.Find(&tcs).Error
}

func (r *TeamCompositionRepository) ByUserId(userId int) (tcs []model.TeamComposition, err error) {
	return tcs, r.store.db.Where("user_id", userId).Find(&tcs).Error
}

func (r *TeamCompositionRepository) ByTeamId(idTeam int) (tcs []model.TeamComposition, err error) {
	return tcs, r.store.db.Where("team_id", idTeam).Find(&tcs).Error
}

func (r *TeamCompositionRepository) ByID(id uint) (ts model.TeamComposition, err error) {
